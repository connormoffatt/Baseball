...
# Get Dataframe of Only Home Runs
...
# Plot histogram of homerun values
library(MASS)
truehist(d.homerun$RUNS.VALUE)

# The most valuable home run is grand slam with 2 outs
subset(d.homerun, RUNS.VALUE == max(RUNS.VALUE))[1,
                  c("STATE", "NEW.STATE", "RUNS.VALUE")]

# Calculate the mean runs created from a homerun
mean.HR <- mean(d.homerun$RUNS.VALUE)

# display the mean home run value
...
# Find the most valuable single
subset(d.single, RUNS.VALUE==max(RUNS.VALUE))[1, 
                  c("STATE", "NEW.STATE", "RUNS.VALUE")]

# Get the elast valuable single
subset(d.single, RUNS.VALUE==min(RUNS.VALUE))[1, 
                  c("STATE", "NEW.STATE", "RUNS.VALUE")]

# calculate mean value and display
mean.single <- mean(d.single$RUNS.VALUE)
abline(v=mean.single, lwd=3)
text(5, 5, "Mean Run Value", pos=4)

#-----------------------------------------

# Calculate dataframe with stealing or caught stealing
stealing <- subset(data2011, EVENT_CD==6 | EVENT_CD==4)
table(stealing$EVENT_CD)

# Calculate states when steals occur
table(stealing$STATE)

# Plot histogram
truehist(stealing$RUNS.VALUE)

# Stealing Runner on first one out
stealing.1001 <- subset(stealing, STATE=="100 1")
table(stealing.1001)

# Get new states after stealing
with(stealing.1001, table(NEW.STATE))

# Get average runs value of stealing
